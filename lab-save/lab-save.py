...
lab_save v2.1 [20260619] with public key authentication
...
def init():
    ...
    a, u, p, e = None, 'username', 'password', 'passenv'
    x = (u, p)
    ...
    for dg in cf['device_groups']:
        ...
        if 'priv_key' in cf[dg] and cf[dg]['priv_key']:
            if 'passphrase' in cf[dg] and cf[dg]['passphrase']:
                passphrase = cf[dg]['passphrase']
            else:
                passphrase = None
            try:
                ctx[dg]['pkey'] = paramiko.RSAKey.from_private_key_file(cf[dg]['priv_key'], password=passphrase)
            except Exception as e:
                ctx['log'].warning(f"{cf[dg]['priv_key']}: {str(e)}")
        elif ctx['pkey']:
            ctx[dg]['pkey'] = ctx['pkey']

        if not ctx[dg]['username'] or (not ctx[dg]['password'] and not ctx[dg]['pkey']):
            ctx['log'].error(f"access undefined or empty")
            ctx['log'].error(f"check {ctx['args'].conf} for details ({dg}.{a})")
            sys.exit(1)
...
def collect_data(host, dg, seq):
    ...
    try:
        ...
        client.connect(
            hostname=host,
            username=ctx[dg]['username'],
            password=ctx[dg]['password'],
            pkey=ctx[dg]['pkey'],
